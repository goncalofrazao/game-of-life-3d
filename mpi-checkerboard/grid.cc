...
void exchange_boudaries(char ***grid, long long local_N[], MPI_Comm grid_comm) {
	...
	//  2D plane with dimensions (x*y), but we want to send vertices received in x_plane and y_plane exchange, so (x+2)*(y+2) blocks with one element
	//  each stride is z+2 - see alloc_grid function
	...
	MPI_Sendrecv(&grid[1][1][1], 1, x_plane, down, 1, &grid[local_N[0] + 1][1][1], 1, x_plane, up, 1, grid_comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&grid[local_N[0]][1][1], 1, x_plane, up, 0, &grid[0][1][1], 1, x_plane, down, 0, grid_comm, MPI_STATUS_IGNORE);
	...
	MPI_Sendrecv(&grid[0][local_N[1]][1], 1, y_plane, right, 0, &grid[0][0][1], 1, y_plane, left, 0, grid_comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&grid[0][1][1], 1, y_plane, right, 1, &grid[0][local_N[1] + 1][1], 1, y_plane, left, 1, grid_comm, MPI_STATUS_IGNORE);
	...
	MPI_Sendrecv(&grid[0][0][local_N[2]], 1, z_plane, front, 0, &grid[0][0][0], 1, z_plane, back, 0, grid_comm, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&grid[0][0][1], 1, z_plane, back, 1, &grid[0][0][local_N[2] + 1], 1, z_plane, front, 1, grid_comm, MPI_STATUS_IGNORE);
