...
// Global array to store the maximum number of cells for each species...
int max_cells[N_SPECIES + 1];
//... and the generation in which it occurred
int generation[N_SPECIES + 1];

char next_state(int x, int y, int z, char ***grid, long long N) {
	// Routine to compute the next state of a cell based on its 26 neighbors and
	// return the new state
	static int ax, ay, az;
	// Variable to store the number of live neighbors
	int sum = 0;
	// Array to store the count of neighbors for each species
	...
	for (int i = -1; i <= 1; i++) {
		...
		for (int j = -1; j <= 1; j++) {
			...
			for (int k = -1; k <= 1; k++) {
				...
				// Increment the count of neighbors for the current species
...
void init_max_cell() {
	// Initialize the max_cells and generation arrays (all elements are set to
	// 0)
...
// Parallelize the counting of cells in initial grid. The grid is divided in
// order to optimize the computation. The reduction clause is used to update the
// max_cells array safely.
...
void get_max(int *cells, int gen) {
	// Update the max_cells and generation arrays, based on the current
	// generation cells count
	for (int i = 1; i <= N_SPECIES; i++) {
...
void simulation(char ***grid, long long N, int generations) {
	// Allocate the auxiliary grid
	char ***new_grid = alloc_grid(N), ***temp;

	// Initialize the max_cells and generation arrays and count gen 0
	init_max_cell();
	count_cells(grid, N);

	for (int i = 0; i < generations; i++) {
		// Initialize the count of cells for each species in the current
		// generation
		int cells[N_SPECIES + 1] = {0};

// Parallelize the simulation. The cube is divided in order to optimize the
// computation. The reduction clause is used update the cells species count
// array safely.
#pragma omp parallel for reduction(+ : cells)
		...
		// Update the max_cells and generation arrays
		get_max(cells, i + 1);
